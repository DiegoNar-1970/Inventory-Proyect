...
import { IoSearch } from "react-icons/io5";
  ...
  const Hours = () => {
    ...
    const {data,loading}=UseBodyFetch(url,{
      method: "POST",
      headers: {
    ...
    const searchFilter = ({ target }) => {
      ...
      Object.entries(data).forEach(([key, hours]) => {
        ...
        const filteredData = hours.data.filter(hora => 
          ...
          hora.employee.profile.name.toLowerCase().includes(target.value.toLowerCase()));
  ...
  return (
    <section className="flex flex-col flex-wrap flex-1 rounded-lg gap-3 text-white">
      ...
      <article className={`flex flex-col bg-fondo-menu rounded-lg box-border p-3 ${view}`}>
              {!formData ? <form action="" className='p-[20px] flex flex-col relative  
               w-[50%] min-w-[305px] max-w-[354px] rounded-xl outline outline-[2px] outline-[#393a3c] ' onSubmit={onSubmit}>
                ...
                <div className="flex gap-5 flex-wrap relative">
                  <div className="flex flex-col gap-2 ">
                    ...
                    {errors.startWeek && <span className="text-red-600">{errors.startWeek.message}</span>}
                  </div>
                  <div className="flex flex-col gap-2 ">
                    ...
                    {errors.endWeek && <span className="text-[15px] text-red-600">{errors.endWeek.message}</span>}
                  </div>
                  <div className="flex flex-col gap-2 ">
